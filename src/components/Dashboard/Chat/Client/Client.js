...
const { REACT_APP_APP_ID, REACT_APP_CHAT_API_KEY, REACT_APP_AGENT_ID, REACT_APP_LOCAL_URL, REACT_APP_PRODUCTION_URL } = process.env;
...
const agentUID = REACT_APP_AGENT_ID;
const CUSTOMER_MESSAGE_LISTENER_KEY = "client-listener";
const limit = 30;

class Client extends Component {
  componentDidMount() {
    addResponseMessage('Welcome to our website! What can we help you with today?');
    addResponseMessage('Let us know below and please wait for our customer support team to get back to you.');
  ...
  fetchAuthToken = async uid => {
    const response = await fetch(api_url+`/chat/auth?uid=${uid}`)
